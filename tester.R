...
#defines and returns a forest on the above training data
update.forest <- function(train_dat){
  train_dat <- train_dat[,2:ncol(train_dat)]
  forest <- randomForest(Action ~.,data=train_dat, mtry=8, importance =TRUE)
  return(forest)
}

#computes and returns our forest's prediction for the test data
test.forest <- function(forest, test_dat){
  forest.guess <- predict(forest, newdata = test_dat)
  return(forest.guess)
}

#Bagging method
#defines and returns bagging on the above training data
update.bagging <- function(train_dat){
  train_dat <- train_dat[,2:ncol(train_dat)]
  bagging <- randomForest(Action ~.,data=train_dat, mtry=ncol(train_dat)-1, importance =TRUE)
  return(bagging)
}

#computes and returns our bagged prediction for the test data
test.bagging <- function(bagging, test_dat){
  bagging.guess <- predict(bagging, newdata = test_dat)
  return(bagging.guess)
}

#KNN Method
#returns normalzed test data
normalizeTest <- function(train, test)
{ 
  newTest <- as.data.frame(scale(test[,2:(ncol(test)-1)], center = sapply(train[,2:(ncol(train)-1)], mean), scale = sapply(train[,2:(ncol(train)-1)], sd)))
  newTest <- cbind(test[,1],newTest, test[,ncol(test)])
  colnames(newTest)[1] <- "Time"
  colnames(newTest)[ncol(test)] <- "Action"
  return(newTest)
}

#returns normalized training data
normalizeTrain <- function(train)
{
  newTrain <- cbind(train[,1],as.data.frame(scale(train[,2:(ncol(train)-1)])),train[,ncol(train)])
  colnames(newTrain)[1] <- "Time"
  colnames(newTrain)[ncol(newTrain)] <- "Action"
  return(newTrain)
}

#computes and returns our knn prediction for the test data
test.knn <- function(train_dat, test_dat){
  train_data_standard = normalizeTrain(train_dat)
  test_data_standard =  normalizeTest(train_dat, test_dat)
  train_dat_restricted = select(train_data_standard, gforcezstmin, gforcezavg, gyroyenergy, speedmax, gforcexavg, gforcexmin, gyroystdev, gyroyenergy)
  test_dat_restricted = select(test_data_standard, gforcezstmin, gforcezavg, gyroyenergy, speedmax, gforcexavg, gforcexmin, gyroystdev, gyroyenergy) 
  
  knn.guess= knn(
    train_dat_restricted,
    test_dat_restricted,
    train_dat$Action, 5
  )
  return(knn.guess)
...
#returns a dataframe containing the times and predictions of random trees and bagging.
produce.dataframe <- function(forest, bagging, test_dat, train_dat){
  prediction <- data.frame("Time" =   test_dat[1],
                   "Forest" = test.forest(forest, test_dat),
                   "Bagging" = test.bagging(bagging, test_dat),
                   "KNN" =  test.knn(train_dat, test_dat)
  )
  prediction <- prediction %>%
    mutate(Aggregate="")
  
  for (i in 1:nrow(prediction)){
    prediction$Aggregate[i] <- getmode(prediction[i,2:4], 3)
  }
  return(prediction)
}

#####################
#End of Modeling Tools
#####################
...
train_data <- read.csv("trainingset.csv")

if (readline(prompt = "Update Model? (Y/N) : ") == "Y") {
  my_forest = update.forest(train_data)
  my_bagging = update.bagging(train_data)
  save(my_forest, my_bagging, file="trees.dat")
  } else {
  load("trees.dat")
  }

guessed_data <- produce.dataframe(my_forest, my_bagging, output_data, train_data)
guessed_data <- guessed_data %>%
  mutate(X = 1)

write.csv(guessed_data, file="predictiondata.csv", row.names = FALSE)

classColors<-c("Bike"="navyblue","Bus"="lightgreen","Car"="#56B4E9","Stationary" ="red2","Unknown" ="#444444","Walk" ="yellow")

p <- ggplot(guessed_data,aes(x=Timestamp/60,y=X,color=Aggregate))
p <- p + geom_point(alpha=1,shape=15,size=5)
p <- p + labs(x="Time (min)", y="")
p <- p + scale_color_manual(values=classColors)
p <- p + theme(panel.background = element_rect(fill="white"),axis.text.y=element_blank(),axis.line.x = element_line())
